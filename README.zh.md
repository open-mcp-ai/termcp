...
| **双传输模式** | Stdio 模式用于本地 MCP 客户端（Claude Code、Cursor）；SSE 模式用于远程部署 |
...
3. **双传输模式**：支持 Stdio 和 SSE over HTTP 两种传输方式。Stdio 模式适用于本地 MCP 客户端（如 Claude Code、Cursor），无需额外启动 HTTP 服务；SSE 模式暴露 HTTP 端点，Agent 可远程连接，实现跨机器部署。
...
检测目标系统上可用的 shell。无需参数。

返回：`{ path, family, hint }`

| 字段 | 类型 | 说明 |
|------|------|------|
| `path` | string | Shell 二进制文件的完整路径（如 `/bin/zsh`、`C:\Windows\System32\cmd.exe`） |
| `family` | string | Shell 家族：`"unix"`、`"powershell"` 或 `"cmd"` |
| `hint` | string | 人类可读的检测来源描述 |

跨平台行为：
- **Unix**：优先读取 `$SHELL` 环境变量；回退顺序 `/bin/zsh` → `/bin/bash` → `/bin/sh`
- **Windows**：优先选择 `pwsh.exe` → `powershell.exe` → `cmd.exe`
...
使用 `go run` 直接从源码运行，每次自动拉取最新版本：

```bash
# Stdio 模式（推荐，用于本地 MCP 客户端）
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--transport` | `sse` | 传输模式：`stdio` 或 `sse` |
| `--host` | `127.0.0.1` | HTTP server 监听地址（仅 SSE 模式） |
| `--port` | `8080` | HTTP server 端口（仅 SSE 模式） |
...
> **提示：** 首次运行会下载并编译源码（需要几秒钟），后续运行使用 Go 构建缓存，启动几乎瞬间完成。

### Claude Code（SSE 模式 — 用于远程部署）
...
- **Stdio 模式：** 任何支持 stdio 传输的 MCP 客户端均可使用 `go run github.com/open-mcp-ai/termcp/cmd/server@latest --transport stdio` 作为启动命令。
- **SSE 模式：** 任何支持 SSE 传输的 MCP 客户端均可连接 `http://<host>:<port>/sse`。
